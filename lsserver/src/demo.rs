...
use crate::{config::ServerConfig, state::StageState};
...
impl DemoAnimator {
    ...
    pub fn tick(&self, state: &mut StageState, config: &ServerConfig) {
        ...
        for arc in 0..config.num_arcs {
            for light in 0..config.lights_per_arc {
                ...
                state.renderer.rgb_fixtures[arc][light].set_color(r, g, b);
                ...
                state.renderer.white_fixtures[arc][light].set_white(w, w, w);
            }
        }

        state.renderer.update(&mut state.current_frame);
    }
}
